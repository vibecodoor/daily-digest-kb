---
name: daily-digest
description: Run the daily AI edge digest for this knowledge base. Use when the user says "run the daily digest", "daily digest", "today's digest", or wants to scout fresh AI/vibe-coding sources and update the KB. Reads the digest workflow, scouts sources, delivers a digest, and promotes durable insights into kb/.
---
...
2. Read `workflows/daily-ai-digest.md` in full — it is the source of truth for
   sources, tiers, freshness, and anti-repeat rules.
3. Read the last ~10 rows of `digests/runs.md` for anti-repeat context.
4. Read `workflows/implicit-intake.md` and `workflows/repo-intake.md` so KB
   promotion follows the intake rules.
...
6. Produce the digest in the shape from `templates/template-digest.md` and
   deliver it to the user. Do not archive it per-day by default — archiving the
   full text to `digests/` is optional.
7. Promote only durable, non-duplicate insights into `kb/`, routing via `MAP.md`
   and shaping each entry with the matching `templates/template-*.md`.
8. Append one row to `digests/runs.md`, ending with the mandatory `Coverage:` line.
